...
STALWART MULTIPHYSICS REALITY ENGINE v4.0.0 (MASTER PRO BUILD)
Description: Dual-Engine Multiphysics Platform with Active Boundary Stabilization,
             ...
             High-Detail Isothermal Streamline Rendering, and ASCII VTK Pipeline.
             [GRAPEFRUIT SECURE MATRIX]
...
# -----------------------------------------------------------------------------
# 1. PAGE CONFIGURATION & STREAMLIT-SAFE TERMINAL STYLING
# -----------------------------------------------------------------------------
st.set_page_config(
...
# -----------------------------------------------------------------------------
# 3. GLOBAL SYSTEM CONTROLS & DUAL-SOLVER SWITCHES
# -----------------------------------------------------------------------------
...
with col_switch2:
    ...
    is_coupled = st.checkbox(
        "🔗 Coupled System Synthesis (Master Sync)", 
        value=False, 
        help="Links all inputs across tabs with real-time feedback and dynamic review tab."
    )
...
def render_detailed_contour_plot(t_matrix, velocity, title_prefix="Thermal Gradient Field Profile"):
    ny, nx = t_matrix.shape
    x = np.linspace(0, nx - 1, nx)
    y = np.linspace(0, ny - 1, ny)
    X, Y = np.meshgrid(x, y)

    fig, ax = plt.subplots(figsize=(11, 4.2), facecolor='#06090e')
    ax.set_facecolor('#06090e')

    # Base Heatmap
    im = ax.imshow(t_matrix, origin='lower', aspect='auto', cmap='magma', extent=[0, nx, 0, ny])

    # Overlay Isothermal Contours
    levels = np.linspace(np.min(t_matrix), np.max(t_matrix), 10)
    cs = ax.contour(X, Y, t_matrix, levels=levels, colors='#00ff66', alpha=0.45, linewidths=0.75)
    ax.clabel(cs, inline=True, fontsize=8, fmt='%1.1f°C', colors='#ffffff')

    # Streamline Flow Direction Vectors
    U = np.ones_like(t_matrix) * velocity
    V = np.zeros_like(t_matrix)
    ax.streamplot(X, Y, U, V, color='#38bdf8', density=0.5, linewidth=0.6, arrowsize=0.8)

    # Colorbar
    cbar = fig.colorbar(im, ax=ax, pad=0.02)
    cbar.set_label('Temperature (°C)', color='#00ff66', fontfamily='monospace', fontsize=9)
    cbar.ax.yaxis.set_tick_params(color='#00ff66')
    plt.setp(plt.getp(cbar.ax.axes, 'yticklabels'), color='#00ff66', fontfamily='monospace', fontsize=8)

    ax.set_title(f"{title_prefix} (Sweep Velocity: {velocity:.2f} m/s)", color='#00ff66', fontfamily='monospace', fontsize=11)
    ax.set_xlabel("X-Axis Spatial Index (Microchannel Length)", color='#00ff66', fontfamily='monospace', fontsize=9)
    ax.set_ylabel("Y-Axis Spatial Index (Wall Gap)", color='#00ff66', fontfamily='monospace', fontsize=9)
    ax.tick_params(colors='#00ff66', labelsize=8)
...
with tab1:
    ...
    with col1:
        fuel_selection = st.selectbox(
            "Fuel Configuration", 
            ["Deuterium-Tritium (D-T)", "Hydrogen-Boron (p-11B)", "Deuterium-Helium-3 (D-3He)", "Custom Matrix"]
        )
        flow_rate_mg = st.number_input(
            "Fuel Mass Flow Rate (mg/s)", 
            value=st.session_state.flow_rate_mg, 
            min_value=0.1, 
            step=0.5, 
            key="flow_rate_input"
        )
...
with tab2:
    ...
    with col1:
        geom_type = st.selectbox(
            "Coil Topology", 
            ["Standard Solenoid", "Helmholtz Pair", "Toroidal Field Array", "Starship Rodin Array", "Custom Coordinates"]
        )
        b_gradient = st.number_input(
            "Field Gradient (∇B) [T/m]", 
            value=st.session_state.b_gradient, 
            step=0.5, 
            key="b_grad_input"
        )
    ...
    st.metric(
        label="Calculated Lorentz Containment Force", 
        value=f"{lorentz_force:.2f} kN", 
        delta="Stable" if lorentz_force < 500 else "Critical Stress"
    )

# -----------------------------------------------------------------------------
# TAB 3: FLUID & THERMAL SOLVER (DETAILED CONTOURS & VTK EXPORT)
# -----------------------------------------------------------------------------
with tab3:
    ...
    with col_in1:
        heat_flux_input = st.number_input(
            "Target Heat Flux (W/cm²)", 
            value=st.session_state.heat_flux_input, 
            step=50.0, 
            key="hf_input"
        )
    ...
    with col_in2:
        flow_velocity = st.slider(
            "Fluid Sweep Velocity (m/s)", 
            0.05, 1.5, 
            value=st.session_state.flow_velocity, 
            step=0.05, 
            key="vel_input"
        )
    ...
    if st.button("RUN THERMAL SIMULATION MATRIX", type="primary"):
        with st.spinner("Computing Sparse Linear System (Advection-Diffusion Discretization)..."):
            ...
            # Render High-Detail Visual
            fig = render_detailed_contour_plot(t_matrix, st.session_state.flow_velocity)
            st.pyplot(fig)
            plt.close()
            ...
            st.download_button(
                label="⬇️ EXPORT .VTK MESH (PARAVIEW / BLENDER READY)", 
                data=vtk_output, 
                file_name="stalwart_thermal_mesh.vtk", 
                mime="text/plain", 
                type="primary"
            )
...
# -----------------------------------------------------------------------------
# TAB 6: FULL SYSTEM REVIEW & HIGH-DETAIL RENDER (DYNAMICALLY GENERATED WHEN COUPLED)
# -----------------------------------------------------------------------------
if is_coupled and tab6 is not None:
    with tab6:
        ...
        fig_r = render_detailed_contour_plot(review_matrix, st.session_state.flow_velocity, title_prefix="Master Coupled Multi-Domain Field Matrix")
        st.pyplot(fig_r)
        plt.close()

# -----------------------------------------------------------------------------
st.caption("STALWART Reality Engine Execution Core v4.0.0 | All Local Arrays Synced. | Grapefruit Verified")
